...
You can install the Syncfusion ASP.NET MVC MCP server as a local tool without a global installation. For guidance on installing and managing local .NET tools, refer to the official documentation: https://learn.microsoft.com/dotnet/core/tools/local-tools-how-to-use.
    ...
    ````json
    {
      "servers": {
        "sf-aspmvc-mcp": {
          "type": "stdio",
          "command": "dotnet",
          "args": ["tool", "run", "syncfusion-aspmvc-mcp"],
          "env": {
            "Syncfusion_API_Key_Path": "YOUR_API_KEY_FILE_PATH"
            // or
            // "Syncfusion_API_Key": "YOUR_API_KEY"
...
The examples below showcase how the `search_docs` tool handles real-world ASP.NET MVC development scenarios. The tool can be invoked directly, as shown in the examples below, for specific needs. Alternatively, an AI assistant can automatically invoke it based on the request.
...
#search_docs Create a Syncfusion ASP.NET MVC Grid with paging, sorting, and filtering enabled.
{% endpromptcard %}
{% endpromptcards %}

{% promptcards %}
{% promptcard RichTextEditor Setup %}
#search_docs How do I set up the Syncfusion ASP.NET MVC RichTextEditor with a custom toolbar and form integration?
{% endpromptcard %}
{% endpromptcards %}
...
To get the most out of the Syncfusion<sup style="font-size:70%">&reg;</sup> ASP.NET MVC MCP Servers:

- **Be Specific** - Include the platform and component in your queries (for example, *"Create a Syncfusion ASP.NET MVC Grid with paging and filtering"*).
...
- **For Troubleshooting** - Use AI suggestions for common issues; consult the [official documentation](https://ej2.syncfusion.com/aspnetmvc/documentation/getting-started/aspnet-mvc-htmlhelper) or [support](https://support.syncfusion.com/support/tickets/create) for complex problems.
