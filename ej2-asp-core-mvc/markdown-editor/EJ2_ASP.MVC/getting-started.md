...
You can add the Syncfusion Markdown Editor to your ASP.NET MVC application by installing the Syncfusion ASP.NET MVC Rich Text Editor package. This can be done in two ways, as mentioned below:
...
Add the **Syncfusion.EJ2** namespace reference to the `Web.config` under the `Views` folder.
...
I> Replace the existing content in the Index.cshtml file by pasting the above code snippet. When the Markdown Editor control is rendered, the id attribute must be present. If the id is not provided, the control will fail to render.
