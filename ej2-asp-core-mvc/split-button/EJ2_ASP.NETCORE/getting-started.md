...
Now, add the ASP.NET Core SplitButton tag helper in `~/Pages/Index.cshtml` page. Populate the action items in `SplitButton` by using the `items` property.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the ASP.NET Core SplitButton control will be rendered in the default web browser.
