...
This section briefly explains how to include the `ASP.NET Core MultiColumn ComboBox` control in an ASP.NET Core application using Visual Studio.
...
Now, add the ASP.NET Core MultiColumn ComboBox tag helper in `~/Pages/Index.cshtml` page.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the ASP.NET Core MultiColumn ComboBox control will be rendered in the default web browser.
