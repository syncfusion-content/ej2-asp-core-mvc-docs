...
Every three months, Syncfusion<sup style="font-size:70%">&reg;</sup> releases new volumes with interesting new features. For this volume, there will be weekly NuGet releases and a service pack. ASP.NET Core JS2 NuGet packages are released on a weekly basis to address critical issue fixes.

From any ASP.NET Core JS2 NuGet version you have installed; you can update to our most recent version.

## Upgrade NuGet packages through Package Manager UI

The NuGet **Package Manager UI** in Visual Studio allows to easily install, uninstall, and update NuGet packages in applications and solutions. You can find and upgrade the ASP.NET Core JS2 NuGet packages to the most recent version or to specific version in the ASP.NET Core solution or application and this process is easy with the steps below:
...
2. The Manage NuGet Packages window will open. Navigate to the **Updates** tab, then search for the ASP.NET Core JS2 NuGet packages using a term like **"Syncfusion"** and select the appropriate ASP.NET Core JS2 NuGet package for your application.
...
There is no distinct command for the update procedure in the Dotnet CLI. Unless you specify the package version, Dotnet CLI installs the latest version of the ASP.NET Core NuGet packages when you use the dotnet add package command.
...
The **Package Manager Console** saves NuGet packages upgrade time since you don't have to search for the package you want to update, and you can just type the command to update the appropriate ASP.NET Core JS2 NuGet package. Follow the steps below to upgrade the installed Syncfusion<sup style="font-size:70%">&reg;</sup> NuGet packages using the Package Manager Console in your ASP.NET Core application.
...
2. The Package Manager Console will be shown at the bottom of the screen. You can install the ASP.NET Core JS2 NuGet packages by entering the following NuGet update commands.

    ***Update specified ASP.NET Core JS2 NuGet package***

    The below command will update the ASP.NET Core JS2 NuGet package in the default ASP.NET Core application

    ```Update-Package <Package Name>```

    **For example:** Update-Package Syncfusion.EJ2.AspNet.Core

    ***Update specified ASP.NET Core JS2 NuGet package in specified ASP.NET Core application***

    The below command will update the ASP.NET Core JS2 NuGet package in the given ASP.NET Core Web application alone
...
4. The NuGet package manager will update the ASP.NET Core JS2 NuGet package as well as the dependencies it has.