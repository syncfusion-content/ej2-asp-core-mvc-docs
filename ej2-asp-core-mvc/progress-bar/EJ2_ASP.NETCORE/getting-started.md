...
This section briefly explains how to include the [ASP.NET Core Progress Bar](https://www.syncfusion.com/aspnet-core-ui-controls/progressbar) control in your ASP.NET Core application using Visual Studio.
...
Add the Syncfusion<sup style="font-size:70%">&reg;</sup> ASP.NET Core Progress Bar tag helper to the `~/Pages/Index.cshtml` page.

{% tabs %}
{% highlight cshtml tabtitle="CSHTML" %}

<ejs-progressbar id="progress"></ejs-progressbar>

{% endhighlight %}
{% endtabs %}

Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. The Syncfusion<sup style="font-size:70%">&reg;</sup> ASP.NET Core Progress Bar control will be rendered in the default web browser.

## Troubleshooting

If the Progress Bar does not render or you run into build/runtime issues, try the following:

* **Progress Bar is not visible on the page** — Ensure the `ejs-scripts` tag helper is registered at the end of `<body>` in `~/Pages/Shared/_Layout.cshtml`. Missing this registration prevents Syncfusion client-side scripts from initializing the control.
