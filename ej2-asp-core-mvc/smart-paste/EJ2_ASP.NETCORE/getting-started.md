...
The theme and script is referred using CDN inside the `<head>` of `~/Pages/Shared/_Layout.cshtml` file as follows,
...
Register the script manager at the end of `<body>` section in the ASP.NET Core application as follows.
...
Now, add the ASP.NET Core Smart Paste Button tag helper in `~/Pages/Index.cshtml` page.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the ASP.NET Core 
