...
Now, add the ASP.NET Core HeatmapChart tag helper in `~/Pages/Index.cshtml` page.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the ASP.NET Core HeatMapChart control will be rendered in the default web browser.
