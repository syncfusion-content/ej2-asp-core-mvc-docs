...
Data binding is a fundamental technique that empowers the Syncfusion ASP.NET Core Grid to integrate data into its interface, enabling the creation of dynamic and interactive Syncfusion ASP.NET Core Grid views. This feature is particularly valuable when working with large datasets or when data needs to be fetched remotely. 

The Syncfusion Syncfusion ASP.NET Core Grid utilizes the **DataManager**, which supports both local binding with JavaScript object arrays and remote binding with RESTful JSON data services. The key property, [dataSource](https://help.syncfusion.com/cr/aspnetcore-js2/Syncfusion.EJ2.Grids.Grid.html#Syncfusion_EJ2_Grids_Grid_DataSource), can be assigned to a DataManager instance or a collection of JavaScript object arrays.
...
The Syncfusion ASP.NET Core Grid offers a loading animation feature, which makes it easy to identify when data is being loaded or refreshed. This feature provides a clear understanding of the Syncfusion ASP.NET Core Grid current state and actions, such as sorting, filtering, grouping, and more.
...
Refreshing the data source in a Syncfusion ASP.NET Core Grid involves updating the data that the v displays dynamically. This operation is essential when you need to reflect changes in the underlying data without reloading the entire page or component.

To achieve this, you can make use of the [dataSource](https://help.syncfusion.com/cr/aspnetcore-js2/Syncfusion.EJ2.Grids.Grid.html#Syncfusion_EJ2_Grids_Grid_DataSource) property in conjunction with the `setProperties` method. This ensures that the Syncfusion ASP.NET Core Grid reflects the changes in the data source without requiring a complete page or component reload.
...
The Syncfusion ASP.NET Core Grid allows dynamic modification of the data source, columns, or both . This feature is particularly valuable when you need to refresh the Syncfusion ASP.NET Core Grid content and structure without requiring a complete page reload.
...
The following code demonstrates how to create the [columns](https://help.syncfusion.com/cr/aspnetcore-js2/Syncfusion.EJ2.Grids.Grid.html#Syncfusion_EJ2_Grids_Grid_Columns) for the Syncfusion ASP.NET Core Grid, which are based on the provided Syncfusion ASP.NET Core Grid data source.
...
The DataTable feature represents a structured table with relational data, equipped with an in-built schema that simplifies working with data column and row objects. This allows for a more intuitive way to manage and display complex data in the Syncfusion Syncfusion ASP.NET Core Grid.

To bind a DataTable to the Syncfusion ASP.NET Core Grid, utilize the [dataSource](https://help.syncfusion.com/cr/aspnetcore-js2/Syncfusion.EJ2.Grids.Grid.html#Syncfusion_EJ2_Grids_Grid_DataSource) property. When you bind a DataTable, Syncfusion ASP.NET Core Grid actions such as Sorting, Filtering, Grouping, and Paging are processed on the client side, enhancing performance and responsiveness.
...
The [On-Demand Grid Actions](#handling-on-demand-grid-actions/) feature in Syncfusion ASP.NET Core Grid enables server-side processing of Syncfusion ASP.NET Core Grid actions, such as sorting, filtering, grouping, and paging. This is especially useful for applications with large datasets, where client-side operations can impact performance.

To implement on-demand server-side actions with a DataTable, you need to:

1. Convert the `DataTable` to an **IEnumerable** object.

2. Use the **DataOperations** class in the controller to process Syncfusion ASP.NET Core Grid actions like sorting, filtering, and paging.
...
Here’s a sample implementation demonstrating how to set up the Syncfusion Syncfusion ASP.NET Core Grid for CRUD operations with a DataTable:
...
In ASP.NET Core, by default the JSON results are returned in camelCase format. So Syncfusion ASP.NET Core Grid field names are also changed in camelCase.
