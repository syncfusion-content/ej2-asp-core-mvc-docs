...
The ASP.NET Core Grid component provides several built-in aggregate types that can be specified in the `type` property to configure an aggregate column.
...
You can use multiple aggregates for a single column in the ASP.NET Core Grid by specifying the aggregate `type` as an array.

Here's an example of how to use multiple aggregates in the ASP.NET Core Grid:
