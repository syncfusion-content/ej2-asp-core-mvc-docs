...
```typescript
@Html.EJS().Button("button1").CssClass("e-info prevbtn").Content("Load Previous Set...").Render()
@Html.EJS().Grid("grid").DataSource((IEnumerable<object>)ViewBag.dataSource).EnableVirtualization().Height("360").BeforeDataBound("beforeDataBound").Columns(col =>
{
    col.Field("OrderID").HeaderText("OrderID").Width("100").TextAlign(Syncfusion.EJ2.Grids.TextAlign.Right).Add();
    ......
...
@Html.EJS().Button("button2").CssClass("e-info nxtbtn").Content("Load Next Set...").Render()
```
...
```
 ...
 <!-- You can view the hosted link for this sample [here](https://ej2.syncfusion.com/aspnetmvc/Load_millions_of_records). -->
