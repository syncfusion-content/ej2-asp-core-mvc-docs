...
@Html.EJS().Grid("grid").DataSource(dataManager => dataManager.Url("https://localhost:xxxx/api/Grid").Adaptor("UrlAdaptor")).AllowPaging(true).AllowSorting(true).AllowFiltering(true).AllowGrouping(true).Toolbar(new List<string>() { "Search" })
    .Columns(col =>
    {
        col.Field("").Type(Syncfusion.EJ2.Grids.ColumnType.Checkbox).Width("50").Add();
        col.Field("OrderID").HeaderText("Order ID").Width("120").TextAlign(Syncfusion.EJ2.Grids.TextAlign.Right).IsPrimaryKey(true).Type(Syncfusion.EJ2.Grids.ColumnType.Number).Add();
        col.Field("CustomerID").HeaderText("Customer ID").Width("140").Type(Syncfusion.EJ2.Grids.ColumnType.String).Add();
        col.Field("ShipCity").HeaderText("ShipCity").Width("140").Add();
        col.Field("ShipCountry").HeaderText("ShipCountry").Width("140").Add();
    })
.GroupSettings(group => group.EnableLazyLoading(true).Columns(new string[] { "CustomerID" })).SelectionSettings(select => PersistSelection(true)).Render();
