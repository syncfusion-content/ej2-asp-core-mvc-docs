...
Now, add the ASP.NET Core ContextMenu tag helper in `~/Pages/Index.cshtml` page and also add target element on which the ContextMenu has to be opened.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the ASP.NET Core ContextMenu control will be rendered in the default web browser.
