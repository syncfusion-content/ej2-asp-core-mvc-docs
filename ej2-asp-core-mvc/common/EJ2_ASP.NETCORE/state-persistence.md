...
The ASP.NET Core library supports persisting a control's state across page refreshes or navigation. To enable this feature, set `enablePersistence` property as true to the required control. This will store the control’s state in browser’s `localStorage` object on page `unload` event.
...
The following table demonstrates the list of ASP.NET Core controls that are supported with state persistence and describes the list of properties stored in the `localStorage`.
