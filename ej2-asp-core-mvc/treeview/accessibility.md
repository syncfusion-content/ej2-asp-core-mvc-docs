...
The TreeView control uses the `tree` role, and each tree parent node has a `group` role where each node has the `treeitem` role. The following ARIA attributes are used in TreeView:
