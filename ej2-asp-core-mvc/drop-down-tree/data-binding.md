...
Dropdown Tree has `load on demand` (Lazy load) option. It reduces payload size when working with the large data. By default, the property `loadOnDemand` is set to false. By enabling this property, it loads first level items initially, and when parent item is expanded, it loads the child nodes based on the `parentValue/child` member.
...
Dropdown Tree can be populated with the hierarchical data source that contains a nested list of data. You can directly map the hierarchical data and the field members with corresponding key values from the hierarchical data to the `fields` property.
...
![Hierarchical datasource Sample](./images/hierarchical-data.PNG)

### Self-referential data

Dropdown Tree can be populated from the self-referential data structure that contains an array of JSON objects with `parentValue` mapping.
...
You can prevent the selection of an individual tree node by using the `Selectable` property. The tree node selection is not allowed while disabling this property.
