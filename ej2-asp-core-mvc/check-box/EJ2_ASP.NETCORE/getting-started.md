...
Now, add the Syncfusion<sup style="font-size:70%">&reg;</sup> ASP.NET Core CheckBox tag helper in `~/Pages/Index.cshtml` page.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the Syncfusion<sup style="font-size:70%">&reg;</sup> ASP.NET Core CheckBox control will be rendered in the default web browser.
...
N> You can also explore our [ASP.NET Core CheckBox example](https://ej2.syncfusion.com/aspnetcore/button/checkbox#/fluent2) that shows you how to render and configure the CheckBox.