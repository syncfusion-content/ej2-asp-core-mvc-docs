...
- `exportBothTableAndChart` (optional): When the [`e-displayOption`](https://help.syncfusion.com/cr/aspnetcore-js2/Syncfusion.EJ2.PivotView.PivotViewDisplayOption.html) property is set to **Both** this exports both table and chart data into a single PDF document.
...
If you want to turn off the repeated row headers in your PDF, set the `allowRepeatHeader` property to **false** inside the [`beforeExport`](https://help.syncfusion.com/cr/aspnetcore-js2/Syncfusion.EJ2.PivotView.PivotView.html#Syncfusion_EJ2_PivotView_PivotView_BeforeExport) event. Make sure you are using the `pdfExport` method with the PivotEngine export.
