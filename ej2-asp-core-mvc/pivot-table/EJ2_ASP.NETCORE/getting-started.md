...
To integrate the ASP.NET Core Pivot Table component, install the required [Syncfusion.EJ2.AspNet.Core](https://www.nuget.org/packages/Syncfusion.EJ2.AspNet.Core/) NuGet packages using the integrated terminal:
...
Now, add the ASP.NET Core Pivot Table tag helper in `~/Pages/Index.cshtml` page.
...
Press <kbd>Ctrl</kbd>+<kbd>F5</kbd> (Windows) or <kbd>⌘</kbd>+<kbd>F5</kbd> (macOS) to run the app. Then, the ASP.NET Core Pivot Table control will be rendered in the default web browser.
