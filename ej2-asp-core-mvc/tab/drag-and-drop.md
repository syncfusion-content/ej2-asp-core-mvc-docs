...
In this example, we have used the Treeview control as an external source, and the child nodes from the Treeview control are dragged and dropped onto the Tab. Therefore, it is necessary to use the `nodeDragStop` event of the Treeview control, where we can form an event object and save it using the `addTab` method of the Tab and remove the dragged node through the `removeNodes` method of Treeview.
